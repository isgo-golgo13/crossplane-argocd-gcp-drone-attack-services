...
gcloud iam workload-identity-pools providers list --location="global" --format="table(name, state)"

echo "Checking IAM Service Account..."
gcloud iam service-accounts list --filter="email:${GCP_IAM_SERVICE_ACCOUNT_EMAIL}" --format="table(email, disabled)"
